...
class GNNRel(nn.Module):
    def __init__(self, L, rel_emb_size, adj_thres=0.05, in_size=256, out_size=256//2):
        super(GNNRel, self).__init__()

        self.in_size = in_size
        self.out_size = out_size

        self.W = nn.Parameter(torch.rand((in_size+rel_emb_size, out_size)))
        self.b = nn.Parameter(torch.rand((out_size, )))

        self.rel_embedding = nn.Parameter(torch.rand((L, rel_emb_size)))
    ...
    # adj_full: [batch, seq, seq, L], where L shows the number of relations
    #           Be careful, sum(seq * L) == 1.0, not sum(L)
    def forward(self, inp, adj_full, is_relu=True):
        # filter too-small values, try not re-scaling first
        adj_full = adj_full.masked_fill(adj_full < self.adj_thres, 0.0)
        ...
        final_repre = torch.cat([neigh_repre, rel_repre], dim=0) # [batch, seq, hidden_dim + emb_dim]
