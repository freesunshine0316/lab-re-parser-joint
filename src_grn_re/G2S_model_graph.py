...
def _clip_and_normalize(word_probs, epsilon):
    ...
    return word_probs / tf.reduce_sum(word_probs, axis=-1, keep_dims=True) # scale preds to sum to 1
...
class ModelGraph(object):
    def __init__(self, word_vocab, char_vocab, pos_vocab, edgelabel_vocab, options, mode='train'):
        ...
        ## [batch, class_num]
        self.distribution = _clip_and_normalize(tf.layers.dense(h_final, options.num_relations,
