...
def read_bionlp_file(inpath, inpath_dep, options):
    ...
    with codecs.open(inpath, 'rU', 'utf-8') as f:
        for g, inst in enumerate(json.load(f)):
            ...
            if options.case == False:
                inst['tokens'] = [x.lower() for x in inst['tokens']]
            all_ids.append(inst['id'])
            all_toks.append(inst['tokens'])
            all_poses.append(inst['poses'])
            si, sj, ei, ej = inst['subj_start'], inst['subj_end']+1, inst['obj_start'], inst['obj_end']+1
            all_entity_indices.append([range(si,sj), range(ei,ej)])
            all_refs.append(inst['ref'].upper() == "TRUE")
    ...
    print('Positive percent: {}'.format(100.0*sum(all_refs)/len(all_refs)))
    return zip(all_toks, all_poses, all_entity_indices, \
...
def collect_vocabs(all_instances, all_words, all_chars, all_poses, all_edgelabels):
    for instance in all_instances:
        toks, poses, in_labels, out_labels = instance[0], instance[1], instance[4], instance[7]
...
class G2SDataStream(object):
    def __init__(self, options, all_instances, word_vocab, char_vocab, pos_vocab, edgelabel_vocab,
        ...
        for (toks, poses, entity_indices, in_neigh, in_label, in_prob, out_neigh, out_label, out_prob,
            ...
            processed_instances.append((toks_idx, toks_chars_idx, poses_idx, entity_indices,
...
#(0-toks_idx, 1-toks_chars_idx, 2-poses_idx, 3-entity_indices,
#                4-in_neigh, 5-in_label_idx, 6-in_prob, 7-out_neigh, 8-out_label_idx, 9-out_prob, 10-ref, 11-id)
        # neigh mask
        ...
        for inst in instances:
            eee = [[1 for x in entity] for entity in inst[3]]
            self.entity_indices_mask.append(eee)
            iii = [[1 for x in in_neigh] for in_neigh in inst[4]]
            self.in_neigh_mask.append(iii)
            ooo = [[1 for x in out_neigh] for out_neigh in inst[7]]
        ...
        self.entity_indices = [x[3] for x in instances] # [batch_size, 2, indices]
        self.in_neigh_indices = [x[4] for x in instances]
        self.in_neigh_edges = [x[5] for x in instances]
        self.out_neigh_indices = [x[7] for x in instances]
        self.out_neigh_edges = [x[8] for x in instances]
        if instances[0][6] != None:
            self.in_neigh_prob = [x[6] for x in instances]
            self.out_neigh_prob = [x[9] for x in instances]
        self.refs = [x[10] for x in instances]
        self.ids = [x[11] for x in instances]
